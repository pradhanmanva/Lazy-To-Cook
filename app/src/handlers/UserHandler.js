...
class UserHandler {
    ...
    register(authCredentials /* : UserAuthenticationModel */) {
        ...
        return dbUtil.getConnection().then(function(connection) {
            ...
            columnValues[USER_TABLE.COLUMNS.ADDRESS] = new String(result.results.insertId);
            return dbUtil.query(result.connection, insertQuery, columnValues);
        }).then(function(result) {
            authenticationColumnValues[USER_TABLE.COLUMNS.ID] = new String(result.results.insertId);
